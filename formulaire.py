...
class FenetreForm(Gtk.Window):
    def __init__(self, fen_win):
        ...
        headerbar.set_title("Formulaire de réservation")
        ...
        self.liste_Classe.insert(1, "1", "E")
        self.liste_Classe.insert(2, "2", "S")
        self.liste_Classe.set_active(0)
        
        #Les Bouttons
        button = Gtk.Button(label="Envoyer")
        button.connect("clicked", self.EnregistrerFormulaire)
        ...
        grid.attach(label_dateReservation, 0, 0, 1, 1)
        grid.attach(self.dateReservation, 1, 0, 1, 1)
        grid.attach(label_dateEntree, 0, 1, 1, 1)
        grid.attach(self.dateEntree, 1, 1, 1, 1)
        grid.attach(label_dateSortie, 0, 2, 1, 1)
        grid.attach(self.dateSortie, 1, 2, 1, 1)
        grid.attach(label_nuite, 0, 3, 1, 1)
        grid.attach(self.nuite, 1, 3, 1, 1)
        grid.attach(label_Classe, 0, 4, 1, 1)
        grid.attach(self.liste_Classe, 1, 4, 1, 1)
        grid.attach(label_numChambre, 0, 5, 1, 1)
        grid.attach(self.numChambre, 1, 5, 1, 1)
        grid.attach(label_tarif_spe, 0, 6, 1, 1)
        grid.attach(self.tarif_spe, 1, 6, 1, 1)
        grid.attach(label_pti_dej, 0, 7, 1, 1)
        grid.attach(self.pti_dej, 1, 7, 1, 1)
        grid.attach(label_phone, 0, 8, 1, 1)
        grid.attach(self.phone, 1, 8, 1, 1)
        grid.attach(label_bar, 0, 9, 1, 1)
        grid.attach(self.bar, 1, 9, 1, 1)
        grid.attach(label_tarif_chambre, 0, 10, 1, 1)
        grid.attach(self.tarif_chambre, 1, 10, 1, 1)
        grid.attach(label_tarif_pti_dej, 0, 11, 1, 1)
        grid.attach(self.tarif_pti_dej, 1, 11, 1, 1)
        grid.attach(label_tarif_phone, 0, 12, 1, 1)
        grid.attach(self.tarif_phone, 1, 12, 1, 1)
        grid.attach(label_tarif_bar, 0, 13, 1, 1)
        grid.attach(self.tarif_bar, 1, 13, 1, 1)
        grid.attach(label_total, 0, 14, 1, 1)
        grid.attach(self.total, 1, 14, 1, 1)
        grid.attach(button, 1, 15, 1, 1)
        
        # Initialize error message label
        self.error_label = Gtk.Label()
        grid.attach(self.error_label, 0, 16, 1, 1)
    ...
    def EnregistrerFormulaire(self, widget):
        ...
        #Fichier Json
        data = {'dateReservation' : dateReservation, 'dateEntree': dateEntree, 'dateSortie': dateSortie, 'nuite': nuite, 'classe': classe, 'tarif_spe': tarif_spe, 'pti_dej': pti_dej, 'phone': phone, 'bar': bar, 'tarif_chambre': tarif_chambre, 'tarif_pti_dej': tarif_pti_dej, 'tarif_phone': tarif_phone, 'tarif_bar': tarif_bar, 'total': total}
